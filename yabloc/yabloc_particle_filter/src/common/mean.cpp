...
namespace
{
  //计算给定角度的加权平均值，使用复数来处理角度的周期性。
  //mean_radian利用复数的性质来计算角度的平均值，这种方法自然地处理了角度的周期性（例如，359度和1度的平均值应为0度）。
...
// 根据粒子集合计算平均姿态。
//get_mean_pose使用粒子的权重计算粒子集的平均位置和姿态。平均姿态通过将每个粒子的方向转换为欧拉角，然后计算加权平均角度来得到。
...
//计算粒子权重的标准差。
//std_of_distribution和std_of_weight分别计算粒子集在空间中的分布标准差和粒子权重的标准差，这些统计量可以帮助理解粒子集的分散程度和权重分布的均匀性。
