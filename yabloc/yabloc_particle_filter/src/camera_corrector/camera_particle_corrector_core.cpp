...
//初始化相机粒子校正器，设置各种参数，并创建必要的发布者和订阅者。
//过监听相机相关的ROS主题（如线段、Lanelet2道路标记、边界框等），并基于这些信息对粒子滤波器中的粒子进行权重调整，以提高定位的准确性。
//这个过程涉及到将视觉信息转化为粒子的权重，从而使得粒子滤波器能够更好地估计车辆的位置。
...
//接收边界框信息的回调函数，将边界框信息设置到cost map中。
...
//接收Lanelet2道路标记信息的回调函数，设置到成本地图中。
