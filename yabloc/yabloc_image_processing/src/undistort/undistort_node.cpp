...
#include <optional>
//对接收到的图像进行畸变校正（去除镜头畸变）并重新发布校正后的图像和相应的相机信息。
...
// 使用先前创建的重映射查找表对图像进行畸变校正，并发布校正后的图像及其相机信息。
