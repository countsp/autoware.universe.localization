...
//这个节点的目的是对来自点云的线段进行过滤，仅保留满足特定几何和视觉条件的线段，然后将这些线段投影到二维图像上，用于后续的处理或可视化。
...
//执行线段过滤和投影的主要逻辑。它处理线段云消息和分割图像消息，过滤线段，并将有效线段投影到图像上。
...
/*    功能：从图像中获取唯一的像素值集合。
    参数：cv::Mat &image，一幅图像。
    返回：std::set<ushort>，包含图像中所有唯一像素值的集合。*/
...
/*  功能：将三维中的线段投影到二维图像平面上。
    参数：
        const pcl::PointCloud<pcl::PointNormal> &points：待投影的线段集合。
        const std::set<int> &indices：要投影的线段索引集合。
        bool negative：是否投影除了给定索引之外的线段。
    返回：pcl::PointCloud<pcl::PointNormal>，投影后的线段集合。*/
...
/*  功能：根据分割图像过滤线段，只保留落在特定区域内的线段。
    参数：
        const cv::Mat &mask：分割图像，用于指定有效区域。
        const pcl::PointCloud<pcl::PointNormal> &edges：待过滤的线段集合。
    返回：std::set<int>，通过过滤的线段索引集合。*/
