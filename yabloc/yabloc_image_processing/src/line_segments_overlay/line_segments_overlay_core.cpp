...
//订阅图像和线段数据消息，首先将接收到的图像按时间戳存储在缓冲区中。
//当接收到线段数据时，查找与之时间戳匹配的图像，然后在该图像上绘制线段。
//线段的颜色根据线段的标签（如可靠或不可靠）进行区分。
//最后，将叠加了线段的图像发布出去，以便进行可视化查看。
...
//处理接收到的图像消息，将其存储在一个缓冲区中，以便与线段数据进行匹配。
...
//处理接收到的线段数据消息，查找与线段时间戳匹配的图像，并在该图像上绘制线段。
