...
//将Lanelet2地图（一种用于自动驾驶和车辆导航的地图格式）分解成更易于处理的形式，如道路标记、标志板和边界框，并发布这些信息。
//将Lanelet2地图元素转换成点云数据和可视化标记
...
//接收并处理Lanelet2地图数据的回调函数。将道路标记、标志板和边界框等信息发布为点云数据和可视化标记。
...
//根据两个点生成包含位置和法线信息的点。,包含起始点位置和由起始点指向终点的法线信息的点。
...
//创建用于可视化Lanelet2地图中的多边形元素的ROS标记数组
